...
NSString *const SystranResponseObjectErrorKey = @"SystranResponseObject";
...
@interface SystranApiClient ()

@property (readwrite, nonatomic) NSDictionary *HTTPResponseHeaders;

@end

@implementation SystranApiClient

- (instancetype)init {
    NSString *baseUrl = [[SystranConfiguration sharedConfig] host];
...
+ (void)initialize {
    if (self == [SystranApiClient class]) {
...
+(NSNumber*) nextRequestId {
    @synchronized(self) {
        ...
        SystranDebugLog(@"got id %ld", nextId);
        return [NSNumber numberWithLong:nextId];
    }
}

+(NSNumber*) queueRequest {
    NSNumber* requestId = [SystranApiClient nextRequestId];
    SystranDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if(matchingItems.count == 1) {
        SystranDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    }
    else {
    ...
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                SystranDebugLog(@"reachability changed to AFNetworkReachabilityStatusUnknown");
                [SystranApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusNotReachable:
                SystranDebugLog(@"reachability changed to AFNetworkReachabilityStatusNotReachable");
                [SystranApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusReachableViaWWAN:
                SystranDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWWAN");
                [SystranApiClient setOfflineState:false];
                break;

            case AFNetworkReachabilityStatusReachableViaWiFi:
                SystranDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWiFi");
                [SystranApiClient setOfflineState:false];
                break;
            default:
                break;
...
- (id) deserialize:(id) data class:(NSString *) class {
   ...
   // model
    ...
    id modelClasscpy = [[ModelClass alloc] initWithDictionary:data error:nil];
    if ( modelClasscpy != nil)
    {
        return [[ModelClass alloc] initWithDictionary:data error:nil];
                ...
                return [[ModelClass alloc] initWithDictionary:[NSJSONSerialization JSONObjectWithData:data options:nil error:nil] error:nil];
    }
    }
    return nil;
}
  ...
  //                                                                     NSString* newStr = [[NSString alloc] initWithData:response encoding:NSUTF8StringEncoding];
                                                                   ...
                                                                   if([self executeRequestWithId:requestId]) {
                                                                       ...
                                                                       if(operation.responseObject) {
                                                                           ...
                                                                           userInfo[SystranResponseObjectErrorKey] = operation.responseObject;
                                                                       }
...
-(NSNumber*)  requestWithCompletionBlock: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [SystranJSONRequestSerializer serializer];
    }
    ...
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [SystranJSONResponseSerializer serializer];
    }
    else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [resourcePath replaceCharactersInRange:[resourcePath rangeOfString:[NSString stringWithFormat:@"%@%@%@", @"{", key, @"}"]]
                                    withString:[SystranApiClient escape:obj]];
    }];
    ...
    if(offlineState) {
        SystranDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        SystranDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        SystranDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [SystranApiClient queueRequest];
        ...
        for(NSString * key in [queryParams keyEnumerator]){
            ...
            if([queryParam isKindOfClass:[NSString class]]){
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [SystranApiClient escape:key], [SystranApiClient escape:[queryParams valueForKey:key]]]];
            }
            else if([queryParam isKindOfClass:[SystranQueryParamCollection class]]){
                SystranQueryParamCollection * coll = (SystranQueryParamCollection*) queryParam;
                NSArray* values = [coll values];
                NSString* format = [coll format];

                if([format isEqualToString:@"csv"]) {
                    [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                        [SystranApiClient escape:key], [NSString stringWithFormat:@"%@", [values componentsJoinedByString:@","]]]];
                    ...
                    for(id obj in values) {
                        [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                            [SystranApiClient escape:key], [NSString stringWithFormat:@"%@", obj]]];
                        counter += 1;
                    }
                ...
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [SystranApiClient escape:key], [NSString stringWithFormat:@"%@", [queryParams valueForKey:key]]]];
            }
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[SystranQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[SystranObject class]]) {
